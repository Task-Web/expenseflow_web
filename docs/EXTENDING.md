...
- **New endpoints**: add route functions in `app/main.py` or a dedicated router module. Reuse `get_user_id` to stay cookie-scoped. Tag endpoints for OpenAPI and update `docs/API.md`.
...
- **ExpenseFlow static API base**: set `expenseflowApiBase` in localStorage or `data-api-base` on `<body>` to point `/api` at a remote backend (localhost defaults to `http://localhost:8000/api` unless overridden).
...
- The ExpenseFlow UI uses PATCH to store submitted reports under `data.expenseflow.submitted_expense_reports`; keep `docs/STATE.md` aligned if you change the payload shape.
...
## Documentation
- Keep `docs/API.md` aligned with actual routes and update examples when shapes change.
